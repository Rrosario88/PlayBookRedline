...
function App() {
  const {
    contractName, playbookName, clauses, results, progress, isAnalyzing, error, demo, useDemo,
    setContractFile, setPlaybookFile, setContractName, setPlaybookName,
    updateRedline, analyze, exportDocx, loadDemo, resetDemo, hydrateMatter,
  } = useAnalysis();
  const { user, authError, authLoading, login, register, logout } = useAuth();
          ...
          <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
            <div className="max-w-3xl">
              <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm font-medium text-slate-100">
                <Scale size={16} /> AI contract review for legal teams
              </div>
              <h1 className="mt-5 text-4xl font-semibold tracking-tight">PlayBookRedline</h1>
              <p className="mt-4 text-lg leading-8 text-slate-300">
                Upload a contract and your negotiation playbook to generate clause-by-clause risk analysis, negotiation-ready redlines, tracked-change DOCX exports, and optional saved matters with explicit retention controls.
              </p>
            </div>
        ...
        <main className="mt-8 space-y-8">
          ...
          <section className="grid gap-6 xl:grid-cols-[1.4fr_0.8fr]">
            <div className="grid gap-6 xl:grid-cols-2">
              <UploadZone
                ...
                onChange={(file) => { resetDemo(); setContractFile(file); setContractName(file?.name || ''); }}
              />
              <UploadZone
                label="Playbook"
                ...
                onChange={(file) => { resetDemo(); setPlaybookFile(file); setPlaybookName(file?.name || ''); }}
              />
            </div>
            <AuthPanel user={user} error={authError} loading={authLoading} onLogin={login} onRegister={register} onLogout={logout} />
          </section>

          {demo && useDemo && (
            <section className="grid gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:grid-cols-2">
              <div>
                <div className="flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.16em] text-slate-500"><FileStack size={16} />Sample contract preview</div>
                <p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-slate-700">{demo.contractPreview}</p>
              </div>
              <div>
                <div className="flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.16em] text-slate-500"><ShieldAlert size={16} />Sample playbook preview</div>
          ...
          {progress && (
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-4 h-3 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-slate-900 transition-all" style={{ width: `${progress.total ? (progress.completed / progress.total) * 100 : 0}%` }} /></div>
            </section>
          )}
            ...
            <>
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-600">Review the AI output, refine redlines, export tracked changes, or save the matter with an explicit retention window.</p>
                </div>
