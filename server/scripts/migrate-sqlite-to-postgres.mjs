...
const databaseUrl = process.env.DATABASE_URL || 'postgresql://playbookredline:playbookredline@127.0.0.1:5432/playbookredline';
...
if (matters.length) {
  await client.query(`SELECT setval(pg_get_serial_sequence('matters','id'), (SELECT MAX(id) FROM matters), true)`);
}
